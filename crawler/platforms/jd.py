...
_JD_REALTY_FEATURE = re.compile(
    # 注意:裸"座"会误命中动产"宝座/座屏",故改为 \d+座|银座 等精确形式;
    # "翡翠"是动产词但"翡翠城"是小区名,靠房产要素(室/幢/苑等)优先保留化解。
    r"室|号楼|幢|栋|弄|\d+座|路\d+|街道|[一-鿿]镇|[一-鿿]乡|[一-鿿]村|小区|花园|花苑|"
    r"公寓|别墅|商铺|店铺|厂房|车位|车库|储藏室|土地|地块|房产|房地产|不动产|宗地|"
    r"楼盘|商业用房|工业用房|名苑|嘉园|新村|公馆|广场|家园|号房|地下室|商住|住宅|"
    r"公租房|安置房|经济适用房|银座|大厦|商厦|写字楼|苑|邨"
)
_JD_MOVABLE_KEYWORD = re.compile(
    r"玉石|首饰|钻石|黄金|铂金|珠宝|手表|腕表|名表|包包|箱包|奢侈品|"
    r"车辆|汽车|轿车|客车|货车|机动车|摩托|电动车|挖掘机|装载机|叉车|机械|"
    r"设备|机床|钻床|加工中心|生产线|流水线|股权|股份|债权|应收|存货|库存|"
    r"原料|材料|钢材|茅台|红酒|字画|国画|油画|书画|古董|文玩|手机|电脑|空调|家电|船舶|"
    r"游艇|商标|专利|线缆|电缆|纺织|服装|"
    # 艺术品/家具/文玩动产(2026-07-05扩充)
    r"底座|宝座|座屏|屏风|摆件|檀木|红木|花梨|花梨木|酸枝|红酸枝|鸡翅木|黑檀|紫檀|"
    r"雕花|瓷器|玉器|镇宅|辟邪|工艺品|艺术品|家具|沙发|茶几|牛角|字画|挂画|画框"
)
...
class JDAuctionCrawler(AbstractBrokerCrawler):
    ...
    async def fetch_detail_api(self, paimai_id: str) -> dict | None:
        ...
        # === 司法拍卖硬过滤（2026-07-05）===
        # 小程序只展示司法拍卖。京东详情 publishSource==7=司法拍卖；9/6 等为店铺资产处置
        # 等非司法标的（court_name 常是"XX拍卖店/XX公司"而非法院）。列表入口 publishSource=7
        # 未被服务端严格执行，会混入非司法，故在详情阶段用权威的 publishSource 硬过滤。
        # 仅当 detail 明确取到 publishSource 且 != 7 才拒绝（detail 接口失败时不误杀）。
        _ps = detail.get("publishSource")
        if _ps is not None and str(_ps) != "7":
            logger.info(f"[JD] 跳过非司法标的 paimai_id={paimai_id} publishSource={_ps}")
            return None
