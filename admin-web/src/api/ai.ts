...
export function deleteAiSession(id: string | number) {
  return http.delete(`/ai/sessions/${id}`).then(r => r.data)
}

export function renameAiSession(id: string | number, title: string) {
  return http.patch(`/ai/sessions/${id}`, { title }).then(r => r.data)
}

export function getSessionMessages(sessionId: string | number) {
