...
import { addFavorite, removeFavorite, getFavorites, getUserProfile, updateUserProfile } from '../../services/user';
...
Page({
  ...
  async loadProperty(id: number) {
    try {
      ...
      this.setData({
        ...
        discount: (property.court_discount_rate && property.court_discount_rate < 1) ? formatDiscount(property.court_discount_rate) : '--',
        discountLabel: '折扣',
  ...
  async checkContactStatus(): Promise<'nologin' | 'need' | 'ok'> {
  ...
  async onSubmitContact() {
    ...
    try {
      await updateUserProfile({ nickname: s, phone: ph });
