...
Page({
    ...
    buildShare() {
        ...
        const parts = [];
        if (d.currentCityId)
            parts.push(`city_id=${d.currentCityId}`);
        if (d.keyword)
            parts.push(`keyword=${encodeURIComponent(d.keyword)}`);
        if (d.selectedDistrict)
            parts.push(`district=${encodeURIComponent(d.selectedDistrict)}`);
        ...
        if (d.presetDiscountMin)
            parts.push(`discount_min=${d.presetDiscountMin}`);
        if (d.presetDiscountMax)
            parts.push(`discount_max=${d.presetDiscountMax}`);
        if (d.presetListedDay)
            parts.push(`listed_day=${d.presetListedDay}`);
        if (d.presetSoldDay)
            parts.push(`sold_day=${d.presetSoldDay}`);
