// 法拍房收益计算器
// 买入法拍房 → 持有 → 二手卖出 的转手差价收益测算。
// 规则为沪甬杭临现行政策常见口径，仅供参考，最终以税务/不动产登记部门核定为准。

import { calcYield, CITY_LIST } from '../../utils/yield-calc';

interface FeeView {
  label: string;
  amount: string;
  note: string;
  isSub?: boolean;   // 保证金等仅展示项
}
...
Page({
  data: {
    ...
    // 输入项（金额单位：元，便于与引擎一致；input 用字符串）
    dealPrice: '',          // 法拍成交价（元）
    deposit: '',            // 保证金（元，仅展示）
    area: '',               // 面积㎡
    repairFundOwed: '',     // 维修基金欠费（元）
    propertyOwed: '',       // 物业欠费（元）
    utilityOwed: '',        // 水电煤欠费（元）
    auctionServiceFee: '',  // 竞拍服务费（元）
    propertyFeeRate: '',    // 物业费单价（元/㎡·月）
    sellPrice: '',          // 二手房卖出总价（元）
    holdYears: '',          // 持有年数

    // 条件开关
    homeCount: 1,           // 1首套 2二套 3三套+
    overTwoYears: true,     // 满2年
    overFiveUnique: false,  // 满五唯一
    incomeMode: 'diff',     // diff差额20% | assess核定1%
    advanceRefundable: true,// 垫付默认可退
  ...
  doCalc() {
    ...
    const city = CITY_LIST[this.data.cityIndex];
    ...
    const r = calcYield(input, cond);

    // 金额统一以「元」显示（千分位），与用户填写单位一致
    const toYuan = (v: number) => {
      const n = Math.round(v);
      return n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    };
    const mapItems = (items: any[]): FeeView[] =>
      items
        .filter((it) => !(it.key === 'deposit'))   // 保证金在结果页不重复展示
        .map((it) => ({
          label: it.label,
